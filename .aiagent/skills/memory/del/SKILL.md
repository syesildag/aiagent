...
1. Call `memory_msearch` with the argument as the query to find matching memories.
2. Show the user the top matches with their IDs and content.
3. Delete the best matching memory (or multiple if clearly all relevant) using `memory_mdelete` with the appropriate ID(s).
