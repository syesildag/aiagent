...
/**
 * Filters the candidate MCP server list for a given prompt using BM25 or
 * embedding similarity over cached tool descriptions.
 *
 * Extracted from AbstractAgent so agents only declare configuration
 * (getName, getSystemPrompt, getAllowedServerNames) and do not contain
 * tool-routing logic.
 */
export class ServerFilter {
   ...
   */
  private buildToolEntries(
    withTools: { name: string }[],
    toolsByServer: Record<string, { function: { description?: string } }[]>,
  ) {
  ...
  private prepareServerFiltering(allowed: string[] | undefined): {
    ...
    const configs = this.serverManager.getEnabledServerConfigs();
   ...
   * Uses cached tool descriptions rather than coarse server-level descriptions.
   * For each server, the maximum similarity across all its tools is used.
   */
